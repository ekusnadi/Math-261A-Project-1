...
$\beta_0 = `r round(coef(model)[1], 1)`$ and slope $\beta_1 = `r round(coef(model)[2], 1)`$. This implies that for a one percentage point increase in the proportion of residents without a high school education, the estimated toxic release score increases by about `r round(coef(model)[2], 0)` units. The model explains only `r round(summary(model)$r.squared * 100, 1)`% of the variation in toxic release ($R^2$ = `r round(summary(model)$r.squared, 3)`), indicating that educational attainment alone accounts for little of the variability in toxic release levels. Although the regression shows a positive correlation between a lower educational attainment of residents and higher toxic waste levels across census tracts in California, the limited explanatory power of the model makes this association uncertain. If this relationship were confirmed, it would suggest that communities with lower educational attainment experience disproportionate exposure to environmental hazards, highlighting environmental inequality. 
