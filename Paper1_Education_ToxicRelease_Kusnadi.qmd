...
$\beta_0 = `r round(coef(model)[1], 1)`$ and slope $\beta_1 = `r round(coef(model)[2], 1)`$. This suggests that for a one percentage point increase in the proportion of residents without a high school education, the estimated toxic release index increases by about `r round(coef(model)[2], 0)` units. The model explains approximately `r round(summary(model)$r.squared * 100, 1)`% of the variation in toxic release ($R^2$ = `r round(summary(model)$r.squared, 3)`), indicating a weak fit overall and suggesting that educational attainment alone does not contribute much to the variability of toxic waste levels. If this relationship were confirmed, it could imply that communities with lower educational attainment are disproportionally exposed to environmental hazards, highlighting environmental inequality. However, while our findings indicate a positive correlation between a lower education attainment of residents and higher toxic waste levels across census tracts in California, a quick look at the scatter plot with the fitted regression line suggests that we should not jump to immediate conclusions.

```{r}
#| echo: false
#| fig-cap: "Scatter plot of Toxic Releases vs. Educational Attainment with fitted regression line (red)."
#| fig-width: 7
#| fig-height: 5
